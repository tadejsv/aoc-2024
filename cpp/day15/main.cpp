...
main() {  // NOLINT
    ...
    while (true) {
        ...
        for (size_t i{ 0 }; i < line.size(); i++) {
            switch (line[i]) {
                case '#':
                    walls.insert(hash_coord(Coords{ curr_line, i }, width));
                    break;
                case 'O':
                    boxes.insert(hash_coord(Coords{ curr_line, i }, width));
                    break;
                case '@':
                    robot = Coords{ curr_line, i };
                    break;
                default:
                    break;
            }
    ...
    const int32_t height{ static_cast<int32_t>(curr_line - 1) };
    ...
    for (const auto& move : moves) {
        bool can_move{ true };
        Coords pos{ robot.first + move.first, robot.second + move.second };
        std::vector<Coords> collected_boxes{};

        while (pos.first >= 0 && pos.first < height && pos.second >= 0 && pos.second < width) {
            ...
            if (boxes.contains((pos_hash))) {
                collected_boxes.push_back(pos);
            } else {
                break;
            }

            pos = { pos.first + move.first, pos.second + move.second };
        }
